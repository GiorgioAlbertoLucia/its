...
class Plotter:
    ...
    def __init__(self, output_dir: Path, output_suffix: str = ''):
    ...
    def plot_training_history(self, train_losses: List[float], 
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'training_history{self.output_suffix}.pdf', 
    ...
    def plot_confusion_matrix(self, labels: np.ndarray, 
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'confusion_matrix{self.output_suffix}.pdf', 
                   dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def analyze_feature_importance_flexible(self,
                                          model: Union[torch.nn.Module, xgb.XGBClassifier, object],
                                          test_results: EvaluationResult,
                                          feature_names: List[str],
                                          class_names_dict: Dict[int, str],
                                          output_file,
                                          ...
                                          model_type: str = 'auto',
                                          momentum_feature_idx: int = None) -> Dict:
        """
        Flexible SHAP analysis that works with multiple model types.
        
        Args:
            model: The trained model (PyTorch, XGBoost, or custom)
            test_results: EvaluationResult containing test data
            feature_names: List of feature names
            class_names_dict: Dict mapping class indices to names
            output_file: ROOT TFile object for saving plots
            max_samples: Maximum number of samples to analyze
            background_samples: Number of background samples for SHAP explainer
            model_type: 'pytorch', 'xgboost', 'ensemble', or 'auto' for auto-detection
            momentum_feature_idx: For ensemble models that need momentum filtering
        ...
        if model_type == 'auto':
            if hasattr(model, 'eval') and hasattr(model, 'parameters'):  # PyTorch
                model_type = 'pytorch'
            elif isinstance(model, xgb.XGBClassifier):
                model_type = 'xgboost'
            elif hasattr(model, 'predict_proba') and hasattr(model, 'regime_models'):  # Ensemble
                model_type = 'ensemble'
            elif hasattr(model, 'predict_proba'):  # Generic sklearn-like
                model_type = 'sklearn'
            else:
                raise ValueError(f"Cannot auto-detect model type for {type(model)}")
        
        print(f"Running flexible SHAP analysis with model type: {model_type}")
        ...
        # Create model wrapper function based on type
        def create_model_wrapper():
            if model_type == 'pytorch':
                device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                model.eval()
                
                def pytorch_wrapper(x: np.ndarray) -> np.ndarray:
                    ...
                    with torch.no_grad():
                        ...
                        probabilities = torch.softmax(outputs, dim=1)
                    return probabilities.cpu().numpy()
                return pytorch_wrapper
                
            elif model_type == 'xgboost':
                def xgboost_wrapper(x: np.ndarray) -> np.ndarray:
                    """Wrapper for XGBoost models."""
                    return model.predict_proba(x)
                return xgboost_wrapper
                
            elif model_type == 'ensemble':
                def ensemble_wrapper(x: np.ndarray) -> np.ndarray:
                    """Wrapper for ensemble models."""
                    return model.predict_proba(x)
                return ensemble_wrapper
                
            elif model_type == 'sklearn':
                def sklearn_wrapper(x: np.ndarray) -> np.ndarray:
        ...
        shap_dir = output_file.mkdir(f'SHAP_analysis_{model_type}')
        shap_dir.cd()
        ...
        for class_idx, class_name in class_names_dict.items():
            ...
            hist_name = f'h_shap_importance_{class_name}_{model_type}'
            hist_title = f'Feature Importance ({class_name}, {model_type});Feature;Mean |SHAP Value|'
            hist = TH1F(hist_name, hist_title, len(feature_names), 0, len(feature_names))
        ...
        print(f"\nFlexible SHAP analysis complete for {model_type} model!")
        print("ROOT histograms saved in output file")

        return results

    def compare_models_shap(self,
                          models_dict: Dict[str, Union[torch.nn.Module, xgb.XGBClassifier, object]],
                          test_results: EvaluationResult,
                          feature_names: List[str],
                          class_names_dict: Dict[int, str],
                          output_file,
        ...
        print(f"Comparing SHAP feature importance across {len(models_dict)} models")
        ...
        for model_name, model in models_dict.items():
            ...
            model_results = self.analyze_feature_importance_flexible(
                model=model,
    ...
    def analyze_xgboost_feature_importance(self,
                                         model: xgb.XGBClassifier,
                                         test_results: EvaluationResult,
                                         feature_names: List[str],
                                         class_names_dict: Dict[int, str],
                                         output_file,
                                         importance_types: List[str] = None) -> Dict:
        """
        Analyze XGBoost built-in feature importance in addition to SHAP.
        ...
        Args:
            ...
            class_names_dict: Dict mapping class indices to names
            output_file: ROOT TFile object for saving plots
            importance_types: List of importance types to compute ('weight', 'gain', 'cover')
        ...
        print(f"Analyzing XGBoost feature importance with types: {importance_types}")
        ...
        xgb_dir = output_file.mkdir('XGBoost_feature_importance')
        xgb_dir.cd()
        
        for importance_type in importance_types:
            importance_values = results['xgboost_importance'][importance_type]
            
            hist_name = f'h_xgboost_importance_{importance_type}'
            hist_title = f'XGBoost Feature Importance ({importance_type});Feature;Importance'
            hist = TH1F(hist_name, hist_title, len(feature_names), 0, len(feature_names))
            
            for feat_idx, (feat_name, importance) in enumerate(zip(feature_names, importance_values)):
                hist.SetBinContent(feat_idx + 1, importance)
                hist.GetXaxis().SetBinLabel(feat_idx + 1, feat_name)
        ...
        print(f"XGBoost feature importance analysis complete!")
        print("ROOT histograms saved in output file")
        
        return results

    def analyze_ensemble_feature_importance(self,
                                          ensemble_model,
                                          test_results: EvaluationResult,
                                          feature_names: List[str],
                                          class_names_dict: Dict[int, str],
                                          output_file,
                                          momentum_feature_idx: int = 0) -> Dict:
        """
        Analyze feature importance for ensemble BDT models.
        
        Args:
            ensemble_model: Trained ensemble model with regime_models attribute
            test_results: EvaluationResult containing test data
            feature_names: List of feature names
            class_names_dict: Dict mapping class indices to names
            output_file: ROOT TFile object for saving plots
        ...
        Returns:
            Dictionary containing ensemble importance analysis results
        """
        
        print("Analyzing ensemble BDT feature importance...")
        ...
        momentum = test_results.features[:, momentum_feature_idx]
        
        regime_importance_data = {}
        for regime_range in ensemble_model.trained_regimes:
            regime_model = ensemble_model.regime_models[regime_range]
            
            mask = ((momentum >= regime_range[0]) & (momentum < regime_range[1]))
            if not np.any(mask):
                continue
            
            importance_dict = regime_model.model.get_booster().get_score(importance_type='gain')
            regime_importance = np.zeros(len(feature_names))
            feature_idx_mapping = 0
            
            for orig_idx, feature_name in enumerate(feature_names):
                if orig_idx == momentum_feature_idx:
                    regime_importance[orig_idx] = 0.0  # Momentum not used in regime models
                else:
                    xgb_feature_key = f'f{feature_idx_mapping}'
                    regime_importance[orig_idx] = importance_dict.get(xgb_feature_key, 0.0)
                    feature_idx_mapping += 1
            
            regime_importance_data[regime_range] = {
                'importance': regime_importance,
                'n_samples': np.sum(mask),
                'momentum_range': regime_range
            }
        
        results['regime_importance'] = regime_importance_data
        
        weighted_importance = np.zeros(len(feature_names))
        
        ensemble_dir = output_file.mkdir('Ensemble_feature_importance')
        ensemble_dir.cd()
        hist_name = 'h_ensemble_average_importance'
        hist_title = 'Ensemble Average Feature Importance;Feature;Weighted Importance'
        hist = TH1F(hist_name, hist_title, len(feature_names), 0, len(feature_names))
        
        for feat_idx, (feat_name, importance) in enumerate(zip(feature_names, weighted_importance)):
            hist.SetBinContent(feat_idx + 1, importance)
            hist.GetXaxis().SetBinLabel(feat_idx + 1, feat_name)
        ...
        regime_dir = ensemble_dir.mkdir('regime_importance')
        regime_dir.cd()
        
        for regime_range, regime_data in regime_importance_data.items():
            regime_name = f"{regime_range[0]:.1f}_{regime_range[1]:.1f}"
            hist_name = f'h_regime_importance_{regime_name}'
            hist_title = f'Feature Importance (p={regime_range[0]:.1f}-{regime_range[1]:.1f} GeV/c);Feature;Importance'
            hist = TH1F(hist_name, hist_title, len(feature_names), 0, len(feature_names))
            
            for feat_idx, (feat_name, importance) in enumerate(zip(feature_names, regime_data['importance'])):
                hist.SetBinContent(feat_idx + 1, importance)
                hist.GetXaxis().SetBinLabel(feat_idx + 1, feat_name)
        ...
        print(f"Ensemble feature importance analysis complete!")
        print("ROOT histograms saved in output file")
